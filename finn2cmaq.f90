program finn2cmaq
  ...
  namelist /control/ chemistry,start_date,end_date,finn_data_directory,griddesc_directory,diurnal_cycle
  ...
  !Leo GRIDDESC:
  call read_GRIDDESC(griddesc_directory, proj, grid)    !(!) TO-DO: mejorar esta funcion basado en lo que haga IOAPI
  ...
  do while (current_date_s <= end_date_s)
           ...
           if ( lati > grid%latmax .or. lati < grid%latmin .or. longi > grid%lonmax .or. longi < grid%lonmin ) then
             ...
             do k=1,nvars
                     ...
                     if ( trim(pollut) == "OC" .or. trim(pollut)  == "BC" .or.  trim(pollut) == "PM25" .or. trim(pollut) == "PM10" ) then
                             emis(k) = emis(k) / 3600000.0      !  kg/day -> g/s    (estrictamente: cuando asigno el ciclo diurno es que se cumple esto)
                             var_units(k)="g/s"             
                     else
                             emis(k) = emis(k) / 3600.0         !mole/day -> mole/s (estrictamente: cuando asigno el ciclo diurno es que se cumple esto)
                             var_units(k) = "mole/s"           
                     endif
                     data(ii,ij,1,1,k) = data(ii,ij,1,1,k) + emis(k)
             enddo
           endif
       enddo
    ...
    do h=1,24
        ...
        write(HH, '(I0.2)') h-1
        outFile="./emis_fires_"//YYYY//DDD//"_"//HH//":00:00_d01.nc"
                   
        tflag(1,:,1) = atoi(YYYY//DDD) 
        tflag(2,:,1) = atoi(HH//"0000")

        print*,"Creando ",outFile
        ...
        do k=1, nvars             
          pollut=var_list(k) 
          call check(nf90_def_var(ncid,trim(pollut)  ,NF90_FLOAT  , [col_dim_id,row_dim_id,lay_dim_id,tstep_dim_id], pollut_var_id)) !
        ...
        call check(nf90_put_att(ncid, pollut_var_id, "units"      , "TFLAG           " ))
        ...
        do k=1,nvars
            ...
            call check(nf90_put_var(ncid, pollut_var_id, data(:,:,:,:,k)*diurnal_cycle(h)  ))  !Escribo valores en NetCDF
        enddo
